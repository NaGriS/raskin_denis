...
public class RS2DOM {
	...
	public static Document ResultSet2XSDDOM(ResultSet rs) throws SQLException {
		...
		Element root = mySchema.createElementNS(xsdns, "schema");
		root.setAttribute("elementFormDefault", "qualified");
		root.setAttribute("attributeFormDefault", "unqualified");
		mySchema.appendChild(root);

		Element result = mySchema.createElementNS(xsdns, "element");
		result.setAttribute("name", "result");
		root.appendChild(result);

		Element ct = mySchema.createElementNS(xsdns, "complexType");
		result.appendChild(ct);

		Element seq = mySchema.createElementNS(xsdns, "sequence");
		ct.appendChild(seq);

		Element row = mySchema.createElementNS(xsdns, "element");
		row.setAttribute("name", "row");
		row.setAttribute("maxOccurs", "unbounded");
		seq.appendChild(row);

		ct = mySchema.createElementNS(xsdns, "complexType");
		row.appendChild(ct);

		seq = mySchema.createElementNS(xsdns, "sequence");
		ct.appendChild(seq);
		...
		for (int i = 1; i <= rsmd.getColumnCount(); i++) {
			element = mySchema.createElementNS(xsdns, "element");
			...
			switch (rsmd.getColumnType(i)) {
				...
				case java.sql.Types.NUMERIC :
					element.setAttribute("type", "decimal");
					break;
